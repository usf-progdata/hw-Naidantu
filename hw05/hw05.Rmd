...
```{r}
Scented_all <- read_excel("Scented_all.xlsx")
g1 <- Scented_all %>% 
  group_by(Date) %>% 
  mutate(averageR = mean(Rating, na.rm = TRUE)) %>% 
  filter(Date >= as.Date("2017-01-01")) %>% 
  ggplot() + 
  aes(x = as.Date(Date), y = averageR, color = as.factor(CandleID)) + 
  geom_point(alpha=.2, shape=1) +
  geom_smooth(method = "loess") + 
  scale_x_date(date_labels='%m-%Y', date_breaks='6 months', name = "Date") + 
  geom_vline(xintercept = as.numeric(as.Date("2020-01-20")), linetype="dashed") +
  scale_color_hue() +
  theme_gray() +
  scale_y_continuous(name = "Average Ratings") +
  guides(colour = guide_legend(title = "CandleID")) +
  labs(title = "Average daily ratings for scented candles") 
...
g2 <- Unscented_all %>% 
  ...
  scale_y_continuous(name = "Average Ratings") +
  guides(colour = guide_legend(title = "CandleID")) +
  labs(title = "Average daily ratings for unscented candles")

g1 + g2
```
Since January 20th in 2020, the average daily ratings of scented candles dropped drastically compared with the average daily rating trend for unscented candles and its' own average daily rating trend before January 20th.
...
candles_full %>% 
  ...
  filter(Date >= as.Date("2017-01-01")) %>% 
  ggplot() + 
  aes(x = as.Date(Date), y = averageR, color = as.factor(Scented), shape = as.factor(Scented)) + 
  geom_point(alpha=.2, shape=1) +
  geom_smooth(method = "loess") + 
  scale_x_date(date_labels='%m-%Y', date_breaks='6 months', name = "Date") + 
  geom_vline(xintercept = as.numeric(as.Date("2020-01-20")), linetype="dashed") +
  ...
  labs(title = "Average daily ratings for candles")
```

Which of these two sets of plots do you find easier to interpret?

The second one that put the rating trends of the two types of candles in one plot, which makes it easier for comparison. 
...
Scented_all %>% 
  ...
  mutate(year = ifelse(Date <= as.Date("2019-12-31"), "2019", "2020")) %>% 
  ggplot() +
  aes(x = as.Date(month), y = noscent, color = as.factor(year)) + 
  stat_summary(aes(y = noscent), fun.data = mean_cl_normal) +
  ...
  labs(x = "Year-Month", title = "Proportion of reviews mentioning that the scented candles have no smell")
```
...
The proportion of reviews mentioning that the scented candles have no smell is increasing and 2020 has a larger increase rate than 2019. 
