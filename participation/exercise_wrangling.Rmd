...
```{r}
gapminder %>%
  filter(continent== "Asia" & pop>1000000)
```
...
```{r}
gapminder %>%
  filter(country== "Brazil" | country== "China" & pop>1000000)
```
...
gapminder %>%
  ...
  mutate(gdpBill=grossGDP/10,0000,0000, gdpBill = sprintf("%0.2f", gdpBill))
```
...
gapminder %>%
  mutate(lifeExp=
           if_else(country=="Canada" & year == 1952, 70, lifeExp))
```
